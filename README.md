...
- `claw stats [--period 24h|7d|30d|all] [--tools] [--agents] [--format table|json]`: 查看当前 ThreadSpace 的 Token、工具调用和 Agent 性能统计
...
当 DuckDB 分析存储启用后，可以直接在 CLI 中查看最近一段时间内的运行指标：

- `claw stats`: 默认显示最近 24 小时的摘要，包括 Token、会话数和消息数
- `claw stats --period 7d`: 查看过去 7 天的数据
- `claw stats --tools`: 查看工具调用次数与成功率
- `claw stats --agents`: 查看各 Agent 的平均响应耗时
- `claw stats --format json`: 输出结构化 JSON，便于后续脚本处理
